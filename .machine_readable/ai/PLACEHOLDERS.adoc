...
sed -i 's/Cookie Rebound/my-project/g' $(grep -rl 'Cookie Rebound' .)
sed -i 's/CookieRebound/MY_PROJECT/g' $(grep -rl 'CookieRebound' .)
sed -i 's/cookie_rebound/my_project/g' $(grep -rl 'cookie_rebound' .)
sed -i 's/cookie-rebound/my-project/g' $(grep -rl 'cookie-rebound' .)
sed -i 's/github.com/github.com/g' $(grep -rl 'github.com' .)
sed -i "s/2026/$(date +%Y)/g" $(grep -rl '2026' .)
sed -i "s/2026-03-28/$(date +%Y-%m-%d)/g" $(grep -rl '2026-03-28' .)
```
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Cookie Rebound` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.guix, devcontainer.json |
| `Persistent cookie vault with rebound-on-wipe` | One-line description | `A tool for X` | flake.guix |
| `CookieRebound` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `cookie_rebound` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `cookie-rebound` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `2026-03-28` | Current date (ISO) | `2026-02-14` | STATE.a2ml, MAINTAINERS.md |
...
| `https://github.com/hyperpolymath/cookie-rebound` | Project website | `https://example.org` | SECURITY.md |
