...
# CookieRebound ABI/FFI Documentation
...
│  generated/abi/cookie_rebound.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcookie_rebound.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── cookie_rebound.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cookie_rebound.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cookie_rebound.h
```
...
```c
#include "cookie_rebound.h"

int main() {
    void* handle = cookie_rebound_init();
    if (!handle) return 1;

    int result = cookie_rebound_process(handle, 42);
    if (result != 0) {
        const char* err = cookie_rebound_last_error();
...
Compile with:
```bash
gcc -o example example.c -lcookie_rebound -L./zig-out/lib
```

### From Idris2

```idris
import CookieRebound.ABI.Foreign
...
```rust
#[link(name = "cookie_rebound")]
extern "C" {
    fn cookie_rebound_init() -> *mut std::ffi::c_void;
    fn cookie_rebound_free(handle: *mut std::ffi::c_void);
    fn cookie_rebound_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cookie_rebound_init();
        assert!(!handle.is_null());

        let result = cookie_rebound_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcookie_rebound = "libcookie_rebound"

function init()
    handle = ccall((:cookie_rebound_init, libcookie_rebound), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cookie_rebound_process, libcookie_rebound), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cookie_rebound_free, libcookie_rebound), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/cookie_rebound.h
   ```
